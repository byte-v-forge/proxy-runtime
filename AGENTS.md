...
本仓是 `proxy-runtime`，只承载统一出口网关、代理上游、代理池和 GOST route runtime 和 Mihomo source runtime 包装能力。

- 本仓可以提供 proxy provider adapter、代理池刷新、出口路由编排、GOST 路由面进程生命周期、Mihomo 订阅源代理面生命周期、健康检查和运行时观测接入点。
...
- 跨仓可复用的代理公开模型来自 `common-lib/proto/byte/v/forge/contracts/proxyruntime/v1/`。
- GOST 与 Mihomo 配置由本仓统一生成；不得在业务仓手写等价运行时配置结构或 provider 参数拼装逻辑。
